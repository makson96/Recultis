...
#manifest_dir = os.getenv("HOME") + "/.steam/steam/SteamApps/"
...
def steamcmd(user):
	...
	if os.path.isfile(engineer_dir+"steamcmd.sh") == False:
		...
		tar.close()
	print(user)
	if os.path.isdir(engineer_dir + "JediAcademy/") == False:
...
def launchers():
	...
	msgBox = QMessageBox.information(qw, "Game is ready", "Have fun!")#, QMessageBox.OK, QMessageBox.OK)
	#msgBox.setText("Game is ready")
	#msgBox.setInformativeText("Have fun!")
...
#	def start_s(self, user):
#		steamcmd(user)
	
	def manifest_and_steam(self):
		print(manifest_dir)
		manifest_file = manifest_dir + "appmanifest_6020.acf"
		...
		manifest.write(""""AppState"
    {
      ...
      "StateFlags" "1026"
		...
		manifest.close()
		os.chmod(manifest_file, stat.S_IRUSR | stat.S_IRGRP | stat.S_IROTH)
		#p_steam = call("steam  -applaunch 6020", executable="/bin/bash", shell=True)
		p_steam = call("steam", executable="/bin/bash", shell=True)
		print("steam closed")
		os.remove(manifest_dir + "appmanifest_6020.acf")
		#symlink()

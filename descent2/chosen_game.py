...
game_dir = recultis_dir + "descent2/"
s_appid = "273580"

name = "Descent 2 on D2X-Rebirth engine"
description = """Descent 2 on D1X-Rebirth engine description
...
screenshot_path = self_dir + "../assets/html/d2xrebirth-screen.png"
desk_dir = str(check_output(['xdg-user-dir', 'DESKTOP']))[2:-3]

def prepare_engine():
	print("prepare engine")
	shutil.copy(recultis_dir + "tmp/dxx-rebirth/d2x-rebirth", game_dir + "d2x-rebirth")
...
def launchers():
	...
	shutil.copy(self_dir + "d2x-rebirth.png", os.getenv("HOME") + "/.local/share/icons/d2x-rebirth.png")
	print("make_launchers")
	shutil.copy(self_dir + "d2x-rebirth.desktop", desk_dir + "/d2x-rebirth.desktop")
	shutil.copy(self_dir + "d2x-rebirth.desktop", os.getenv("HOME") + "/.local/share/applications/d2x-rebirth.desktop")

def start(shop, shop_login, shop_password):
	print("start install d2x-rebirth")
...
def uninstall():
	if os.path.isfile(desk_dir + "/d2x-rebirth.desktop"):
		os.remove(desk_dir + "/d2x-rebirth.desktop")
	if os.path.isfile(os.getenv("HOME") + "/.local/share/applications/d2x-rebirth.desktop"):
		os.remove(os.getenv("HOME") + "/.local/share/applications/d2x-rebirth.desktop")
	if os.path.isfile(os.getenv("HOME") + "/.local/share/icons/d2x-rebirth.png"):
		os.remove(os.getenv("HOME") + "/.local/share/icons/d2x-rebirth.png")
